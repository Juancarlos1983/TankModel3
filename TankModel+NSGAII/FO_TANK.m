function f = FO_TANK(x)

%% function f = evaluate_objective(x)
% x é o vetor dos parametros.

f = [];
[Q,QO] = TANK3(x);   %A vazão calculada (Q) e a vazão observada (Q0) são 
%retornados após rodar o modelo IPHII para que se realize o cálculo das 
%funções objetivo.
%% Calculo das funções objetivo
...
%CALCULO DAS FO -----------------------------
% % OF(1)=1.-KGE;                       %RMSE (Kling-Gupta - UNIDADES DE M3/S) ********
% % OF(2)=SF/nsf;                       %DESVIO ABSOLUTO MEDIO
% % OF(2)=SF1/nsf;                      %DESVIO QUADRADO RELATIVO MEDIO
OF(2)=(SF2/nsf)^0.5;                    %UNIDADES DE 1/M3/S (AVALIA VAZÕES BAIXAS) ***************
OF(1)=1. - (1. -(SFNS/somnash));        %COEFICIENTE DE NASH-SUTCLIFFE: OF(1)=1.-(SFNS/somnash);
% para a minimização.
% % OF(6)=SF3/nsf;                      %DESVIO RELATIVO MEDIO
% % OF(7)=abs((SQ/sobs-1.)*100.);       %ERRO NO VOLUME - UNIDADE %  PARA CALIBRACAO MANUAL
